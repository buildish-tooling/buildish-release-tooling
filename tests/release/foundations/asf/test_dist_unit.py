...
class AsfSvnUnitTest(unittest.TestCase):
    ...
    def test_checkout_url_runs_expected_command(self) -> None:
        client = AsfSvnClient()
        with mock.patch(
            "buildish_release_tooling.release.foundations.asf.dist.run_logged_command",
            return_value=subprocess.CompletedProcess([], 0, "", ""),
        ) as run_command:
            client.checkout_url(
                "https://example.invalid/repos/dist/dev/incubator/example",
                Path("/sandbox/wc"),
            )
        run_command.assert_called_once_with(
            ["svn", "checkout", "https://example.invalid/repos/dist/dev/incubator/example", "/sandbox/wc"],
