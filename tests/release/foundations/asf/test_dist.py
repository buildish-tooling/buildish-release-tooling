...
class AsfSvnIntegrationTest(unittest.TestCase):
    ...
    def test_asf_svn_can_stage_promote_and_prune(self) -> None:
        ...
        dev_base_url = url_join(repo_url, "dist/dev/incubator/example", component_id)
        release_base_url = url_join(repo_url, "dist/release/incubator/example", component_id)
        ...
        artifact_path = sandbox_dir / "apache-example-project-1.2.3-incubating-src.tar.gz"
        ...
        client.working_copy_put_file(
            working_copy_dir,
            artifact_path,
            f"dist/dev/incubator/example/{component_id}/1.2.3-rc0/{artifact_path.name}",
        )
        client.working_copy_put_file(
            working_copy_dir,
            artifact_path,
            f"dist/release/incubator/example/{component_id}/1.2.1/apache-example-project-1.2.1-incubating-src.tar.gz",
        )
