...
class ComponentCase:
    """Expected release behavior for one neutral ASF profile fixture."""
...
COMPONENT_CASES = (
    ComponentCase(
        fixture_id="buildish-mammoth-cache",
        component_id="example-action",
        version="1.2.3",
        release_line="1.2.x",
    ...
    ComponentCase(
        fixture_id="buildish-no-gradle-wrapper-jar",
        component_id="example-script",
        version="1.2.3",
        release_line="1.2.x",
    ...
    ComponentCase(
        fixture_id="buildish-site-pipeline",
        component_id="example-site",
        version="1.2.3",
        release_line="1.2.x",
        prepare_rc_tags=(),
            ...
            with self.subTest(component=case.component_id):
                ...
                completed = run_cli(
                    [
                        "prepare-rc",
                        "--component-config",
                        str(fixture_component_config_path(case.fixture_id)),
                        case.version,
                    ],
                    cwd=clone_dir,
        ...
        for case in COMPONENT_CASES:
            with self.subTest(component=case.component_id):
                ...
                config_path = write_fixture_component_config(
                    ...
                    asf_dist_dev_base=f"{repo_url}/dist/dev/incubator/example/{case.component_id}",
                    asf_dist_release_base=f"{repo_url}/dist/release/incubator/example/{case.component_id}",
                )
                client = AsfSvnClient()
                release_base_url = f"{repo_url}/dist/release/incubator/example/{case.component_id}"
            ...
            with self.subTest(component=case.component_id):
                ...
                completed = subprocess.run(
                    [
                        ...
                        str(fixture_component_dispatcher_path(case.fixture_id)),
                        "prepare-rc",
                        case.version,
                    ],
                    cwd=str(clone_dir),
