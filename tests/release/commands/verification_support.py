        ...
        if self.include_generic_file:
            lines.extend(
                [
                    ...
                    "        output_globs:",
                    "          - dist/example-project-bootstrap.zip",
                    "      comparison:",
                    "        mode: exact-bytes",
                ]
            )
        ...
        if self.include_oci_image:
            lines.extend(
                [
                    ...
                    "        image_ref: ghcr.io/apache/example-project:rebuild-local",
                ]
            )
...
class VerificationCommandsIntegrationTestBase(ReleaseCommandsIntegrationTestSupport):
                ...
                rebuild_script.write_text(
                    "\n".join(
                        [
                            ...
                            "import zipfile",
                            "archive_path = Path('dist/example-project-bootstrap.zip')",
                ...
                rebuild_script.write_text(
                    "\n".join(
                        [
                            ...
                            "from pathlib import Path",
                            "import tarfile",
                            "archive_path = Path('dist/example-project-1.2.3.tgz')",
    ...
    def _prepare_verification_fixture(
        ...
        *,
        ...
        release_dir = sandbox_dir / "dist" / "release" / "incubator" / "example"
        ...
        if secondary_kind is not None:
            secondary_name = "example-project-bootstrap.zip"
            ...
            if include_second_generic_file_shared_profile:
                second_secondary_name = "example-project-bootstrap-alt.zip"
        ...
        if include_npm_package:
            artifact_file_path = sandbox_dir / "npm-dist" / "example-project-1.2.3.tgz"
            ...
            metadata_dir = registry_root / "@apache" / "example-project"
            metadata_dir.mkdir(parents=True, exist_ok=True)
            (metadata_dir / "index.json").write_text(
                json.dumps(
                    {
                        "name": "@apache/example-project",
                        "versions": {
                            "1.2.3": {
                                "name": "@apache/example-project",
                                "version": "1.2.3",
        ...
        manifest_payload: dict[str, object] = {
            ...
            "draft_github_release": {
                "repository": "apache/example-project",
                "tag": rc_tag,
                "url": f"https://github.com/apache/example-project/releases/tag/{rc_tag}",
            },
            "vote_materials": {
                "source_artifacts": [
                    {
