...
class VoteMaterialsCommandsIntegrationTest(ReleaseCommandsIntegrationTestSupport):
    ...
    def _build_finalize_svn_repo_template(cls) -> Path:
        ...
        dev_base_url = f"{repo_url}/dist/dev/incubator/example/{component_id}"
        release_base_url = f"{repo_url}/dist/release/incubator/example/{component_id}"
        keys_path = (
            working_copy_dir / "dist" / "release" / "incubator" / "example" / "KEYS"
        )
    ...
    def test_finalize_rc_vote_materials_command_stages_manifest_and_mirrors_it(
        ...
        bootstrap_asset_path = sandbox_dir / "example-project-bootstrap.zip"
        ...
        completed = run_cli(
            [
                ...
                "--sha512-uri",
                "https://github.com/apache/example-project/releases/download/v1.2.3/example-project-bootstrap.zip.sha512",
                "--artifact-origin",
                "source-commit",
                "--git-commit-sha",
        ...
        self.assertEqual(
            "https://github.com/apache/example-project/releases/tag/v1.2.3-rc0",
