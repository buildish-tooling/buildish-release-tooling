...
class RcPreparationCommandsIntegrationTest(ReleaseCommandsIntegrationTestSupport):
    ...
    def test_prepare_rc_command_uses_yaml_component_config(self) -> None:
        ...
        self.assertEqual(
            expected_source_date_epoch, github_outputs["source_date_epoch"]
        )

    def test_cleanup_dev_svn_rcs_command_deletes_matching_version_directories(
        self,
    ) -> None:
    ...
    def test_prepare_rc_rejects_non_production_release_targets_without_opt_in(
        self,
    ) -> None:
        sandbox_dir = create_build_test_sandbox()
        self.addCleanup(cleanup_sandbox, sandbox_dir)
    ...
    def test_prepare_rc_allows_non_production_release_targets_with_opt_in(self) -> None:
        ...
        self.assertEqual(
            "file:///tmp/buildish-test/dev/1.2.3-rc0/", manifest["staging_url"]
        )
