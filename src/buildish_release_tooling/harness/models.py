...
class SvnRepositoryFixture(ConsumerOwnedAuthoredModel):
    ...
    initial_state: SvnInitialState = Field(
        default="absent",
    ...
    version: str | None = Field(
        default=None, description="Release version string without a leading `v` prefix."
    )
    rc_number: int = Field(
...
class InvocationMatch(ConsumerOwnedAuthoredModel):
    ...
    env_contains: dict[str, str] = Field(
...
class ToolBehaviorResult(ConsumerOwnedAuthoredModel):
    ...
    writes: list[FileWriteAction] = Field(
...
class HarnessBuiltinGhReleaseAsset(RuntimeDerivedModel):
    """Synthetic GitHub Release asset retained by the stateful harness shim."""

    id: int = Field(ge=1, description="Synthetic GitHub Release asset identifier.")
    name: str = Field(description="Release asset basename.")
    size: int = Field(ge=0, description="Release asset size in bytes.")
    digest: str = Field(description="GitHub-style algorithm-prefixed asset digest.")
    stored_path: str = Field(
        description="Workspace-relative path containing the retained bytes."
    )


class HarnessBuiltinGhRelease(RuntimeDerivedModel):
    """Synthetic GitHub Release retained by the stateful harness shim."""

    id: int = Field(ge=1, description="Synthetic GitHub Release identifier.")
    repository: str = Field(
        description="GitHub repository identity associated with the release."
    )
    tag_name: str = Field(description="Exact tag associated with the release.")
    name: str = Field(description="Release title.")
    body: str = Field(description="Release body.")
    draft: bool = Field(description="Whether the release remains a draft.")
    prerelease: bool = Field(description="Whether the release is a prerelease.")
    html_url: str = Field(description="Synthetic browser-facing release URL.")
    url: str = Field(description="Synthetic API-facing release URL.")
    assets: list[HarnessBuiltinGhReleaseAsset] = Field(
        default_factory=list,
...
class HarnessShimState(RuntimeDerivedModel):
    ...
    @classmethod
    def validate_tool_behavior_names(
        cls, value: dict[str, list[ToolBehavior]]
    ) -> dict[str, list[ToolBehavior]]:
...
class WorkflowScenario(ConsumerOwnedAuthoredModel):
    ...
    event: Literal["workflow_dispatch"] = Field(
...
class HarnessScenario(ConsumerOwnedAuthoredModel):
    ...
    backend: HarnessBackendName = Field(
        default="custom",
    ...
    git_repositories: list[GitRepositoryFixture] = Field(
    ...
    def validate_job_graph(self) -> HarnessScenario:
        ...
        for job in self.jobs:
            ...
            if unknown_needs:
                raise ValueError(
                    f"job {job.id} references unknown needs: {', '.join(unknown_needs)}"
                )
        return self
...
class HarnessRunResultJson(RuntimeDerivedModel):
    ...
    inspectable_paths: HarnessInspectablePaths = Field(
...
class HarnessSequenceEntryJson(RuntimeDerivedModel):
    ...
    scenario: str = Field(
        description="Scenario name associated with the related harness sequence result entry."
    )
    workspace: str = Field(
        description="Filesystem path of the harness workspace directory for the related run."
    )
...
class HarnessSequenceRunResultJson(RuntimeDerivedModel):
    ...
    sequence: list[HarnessSequenceEntryJson] = Field(
        description="Ordered per-scenario results retained for one multi-scenario harness sequence run.",
        default_factory=list,
    )
    final_workspace: str = Field(
        description="Filesystem path of the final harness workspace retained after a multi-scenario sequence run."
    )
...
HarnessShimState.schema_export = SchemaExportSpecification(
    filename="harness-shim-state.schema.json",
    audience="internal",
    stability="stable",
...
HarnessScenario.schema_export = SchemaExportSpecification(
    filename="harness-scenario.schema.json",
    audience="internal",
    stability="stable",
...
HarnessCommandTraceEntry.schema_export = SchemaExportSpecification(
    filename="harness-command-trace-entry.schema.json",
    audience="internal",
    stability="stable",
...
HarnessRunResultJson.schema_export = SchemaExportSpecification(
    filename="harness-run-result-json.schema.json",
    audience="internal",
    stability="stable",
...
HarnessSequenceRunResultJson.schema_export = SchemaExportSpecification(
    filename="harness-sequence-run-result-json.schema.json",
    audience="internal",
    stability="stable",
