...
def write_workspace_file(
    root: Path, relative_path: str, content: str, executable: bool
) -> None:
...
def resolve_workspace_relative_path(root: Path, relative_path: str) -> Path:
    ...
    if ".." in candidate.parts:
        raise ValueError(
            f"workspace file paths must not escape the workspace: {relative_path}"
        )
...
def write_shim_state(
    workspace: HarnessWorkspace,
    scenario: HarnessScenario,
    *,
    seed_state: HarnessShimState | None = None,
) -> None:
    ...
    state = HarnessShimState(
        ...
        gh_releases={} if seed_state is None else seed_state.gh_releases,
        gh_next_release_id=(1 if seed_state is None else seed_state.gh_next_release_id),
        gh_next_asset_id=1 if seed_state is None else seed_state.gh_next_asset_id,
    )
...
def write_bash_env_hook(workspace: HarnessWorkspace, scenario: HarnessScenario) -> None:
    ...
    tools = sorted(
        set(scenario.tool_behaviors) | {"gh", "docker", "gpg", "java", "javac"}
    )
    lines = [
        "#!/usr/bin/env bash",
...
def write_job_statuses(
    workspace: HarnessWorkspace, statuses: dict[str, HarnessJobStatus]
) -> None:
...
def remove_workspace(workspace_root: Path) -> None:
    ...
    if not resolved_workspace_root.name.startswith("scenario."):
        raise ValueError(
            f"refusing to delete non-harness workspace path: {workspace_root}"
        )
