...
def render_release_name_template(
    template: str,
    *,
    component: str,
    version: str,
    ...
    """Expand and validate a provider-neutral release filename or archive-root template."""

    require_semantic_version(version)
    try:
        rendered = template.format(component=component, version=version)
    except (IndexError, KeyError, ValueError) as error:
        raise ValueError(f"invalid {field_name} template: {template}") from error
    return validate_simple_filename(rendered, field_name=field_name)
